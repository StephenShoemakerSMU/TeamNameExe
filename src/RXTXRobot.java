...
public abstract class RXTXRobot extends SerialCommunication
{
    ...
    {
        this.getOutStream().println("Connecting to robot...\n");
        if ("".equals(getPort()))
        {
    ...
    */
    ...
    public int getConductivity()
    {
     ...
     /**
     * Gets the result from the IR sensor (must be connected into digital pin 4)
     * 
     * @return 
     */
    public char getIRChar()
    {
        if (!isConnected())
        {
            error("Robot isn't connected!", "RXTXRobot", "getIRChar");
            return '0';
        }

        this.attemptTryAgain = true;
        String response = this.sendRaw("i", 700);
        String[] arr = response.split("\\s+");
        this.attemptTryAgain = false;

        if (arr.length != 2)
        {
            error("Incorrect response from Arduino (Invalid length)!: " + response, "RXTXRobot", "getIRChar");
            debug("IR Sensor Response: " + response);
            return '0';
        }
        ...
        {
            error("Incorrect response from Arduino (Invalid datatype)!: " + response, "RXTXRobot", "getIRChar");
            debug("IR Sensor Response: " + response);
        }
...
//              
    ...
    /**
     *  ------------------------------
        Controlling motors and servos on PCA9685 Board
        ------------------------------
        X [channel] // Stops a PWM signal on [channel]. // pcaStop()
 
        t [channel] [degree] [duration] // Move a 180deg Servo to degree, 0 <= degree <= 180+ (for student calibration), for [duration] in milliseconds.
        T [channel] [speed] [duration]// Move a continuous servo on [channel] at [speed], [speed] = 0 to stop, -90 <= speed <= 90, for [duration] in milliseconds.

        ? [digitalPin] // Query encoder ticks for a given [digitalPin]
     * 
     */
    
    public void runPCAServo(int channel, int position)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "runPCAServo");
            return;
        }

        debug("Moving 180 servo on channel" + channel + " to position " + position);
        if (!getOverrideValidation() && (position < 0 || position > 180))
        {
            error("Position must be >=0 and <=180.  You supplied " + position + ", which is invalid.", "RXTXRobot", "moveServo");
        } else
        {
            sendRaw("s " + channel + " " + position);
        }

    }
    
    public void runPCATimedServo(int channel, int position, int duration)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "runPCAServo");
        ...
        debug("Moving 180 servo on channel" + channel + " to position " + position + " for " + duration + " milliseconds.");
        if (!getOverrideValidation() && (position < 0 || position > 180))
        {
            error("Position must be >=0 and <=180.  You supplied " + position + ", which is invalid.", "RXTXRobot", "moveServo");
        } else
        {
            sendRaw("t " + channel + " " + position + " " + duration);
        }

    }
    
    public void runPCATimedContServo(int channel, int speed, int duration)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "runMotor");
        ...
        debug("Moving cont servo on channel" + channel + " with speed " + speed + " for " + duration + " milliseconds.");
        if (!getOverrideValidation() && (speed < -90 || speed > 90))
        {
            error("Speed must be >= -90 and <= 90.  You supplied " + speed + ", which is invalid.", "RXTXRobot", "moveServo");
        } else
        {
            sendRaw("T " + channel + " " + speed + " " + duration);
        }
    }
    
    public void runPCAContServo(int channel, int speed)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "runMotor");
        ...
        debug("Moving cont servo on channel" + channel + " with speed " + speed + ".");
        if (!getOverrideValidation() && (speed < -90 || speed > 90))
        {
            error("Speed must be >= -90 and <= 90.  You supplied " + speed + ", which is invalid.", "RXTXRobot", "moveServo");
        } else
        {
            sendRaw("S " + channel + " " + speed);
        }
    }
     ...
     * blocking method).
     *
     * Accepts a channel with a DC motor,
     * the speed that the motor should run at
     * [-1023 to 1024], and the time with which the motor should run (in
     * milliseconds, with a range of 0 and 60000. 
     * If speed is negative, the motor will run in
     * reverse. 
     * If time is 0, the motor will run infinitely until
     * another call to that motor is made, even if the Java program terminates.
     * To stop a motor from turning, use the following method call:
     * robot.runMotor({channel}, 0, 0);
     * 
     */
    
    public void runPCAMotor(int channel, int speed, int time)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "runMotor");
        ...
        if (!getOverrideValidation() && (time < 0 || time > 60000))
        {
            error("Method was not given a time that is 0 <= time <= 30000.", "RXTXRobot", "runMotor");
            return;
        }
        
        if (time != 0)
        {
            debug("Running motor on channel " + channel + " at speed "+speed+" for " + time + " milliseconds.");
            sendRaw("u " + channel + " " + speed + " " + time);
        }
        
        else if (time == 0)
        {
           debug("Running motor on channel " + channel + " at speed "+ speed +".");
           sendRaw("w " + channel + " " + speed);
        }
    }
    
    /**
     * Runs two DC motors at various specific speeds for a specific time (Potential
     * blocking method).
     *
     * Accepts a two channels with two DC motors,
     * the speeds that the motors should run at
     * [-1023 to 1024], and the time with which the motor should run (in
     * milliseconds, with a range of 0 and 60000. 
     * If speed is negative, the motors will run in
     * reverse. 
     * If time is 0, the motor will run infinitely until
     * another call to that motor is made, even if the Java program terminates.
     * To stop a motor from turning, use the following method call:
    ...
    public void runTwoPCAMotor(int channel1, int speed1, int channel2, int speed2, int time)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "runMotor");
        ...
        if (!getOverrideValidation() && (time < 0 || time > 60000))
        {
            error("Method was not given a time that is 0 <= time <= 60000.", "RXTXRobot", "runMotor");
            return;
        }
 
        if (time != 0)
        {
           debug("Running motor 1 on channel " + channel1 + " at speed "+ speed1 +" and motor 2 on channel " + channel2 + 
           "at speed " + speed2 + " for " + time + " milliseconds.");
           sendRaw("U " + channel1 + " " + speed1 + " " + channel2 + " " + speed2 + " " + time);
        }
        ...
        else if (time == 0)
        {
           debug("Running motor 1 on channel " + channel1 + " at speed "+ speed1 +" and motor 2 on channel " + channel2 + 
           "at speed " + speed2 + " for " + time + " milliseconds.");
           sendRaw("W " + channel1 + " " + speed1 + " " + channel2 + " " + speed2);
        }
    }
    
    public void runPCAEncoder(int channel, int speed, int pin, int ticks)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "runMotor");
        ...
        debug("Running motor on channel " + channel + " at speed " + speed +" for " + ticks + " ticks with encoder on pin " + pin +".");
        sendRaw("y " + channel + " " + speed + " " + pin + " " + ticks);
        
    }
    
    public void runTwoPCAEncoder(int channel1, int speed1, int pin1, int ticks1, int channel2, int speed2, int pin2, int ticks2)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "runMotor");
        ...
        debug("Running motor 1 on channel " + channel1 + " at speed " + speed1 +" for " + ticks1 + 
                " ticks with encoder on pin " + pin1 +" and running motor 2 on channel " + channel2 + 
                " at speed " + speed2 +" for " + ticks2 + " ticks with encoder on pin " + pin2 +".");
        sendRaw("Y " + channel1 + " " + speed1 + " " + pin1 + " " + ticks1 + " " 
                + channel2 + " " + speed2 + " " + pin2 + " " + ticks2);
    }
    
    public int getPCAEncodedMotorTicks(int channel)
    {
        if (!isConnected())
        {
            error("Robot is not connected!", "RXTXRobot", "getEncodedMotorPosition");
        ...
        debug("Checking tick position for encoder on channel " + channel);
        this.attemptTryAgain = true;
        String[] split = sendRaw("? " + channel).split("\\s+");
        this.attemptTryAgain = false;
        if (split.length != 3)
        {
            error("Incorrect length returned: " + split.length, "RXTXRobot", "getEncodedMotorPosition");
    ...
    /**
     * Stops specific motor.
     *
     * Simply sends a command to the robot 
     * to stop the motor on the given channel.
     * 
     */
    
    
    public void channelPCAStop(int channel)
    {
        debug("Stopping motor on channel " + channel + ".");
        sendRaw("X " + channel);
    }
     ...
     * 
     * Simply sends a command to the robot
     * to stop all motors (both servo and DC) running.
     * 
     */
    
    public void allPCAStop()
    {
